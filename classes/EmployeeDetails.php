...
//$sql = "insert into tblContactus(Fname,Lname,Organization,Designation,WorkPhone,City,Country,Description,".
       // "Email,flag) values('$rfname','$rlname','$rorganization','$rdesignation','$rphone','$rcity','$rcountry',".
      //  "'$rdescription','$remail','$rflag')";

$sql = "insert into tblEmpVerfication(FirstName,LastName,Organization,Designation,EmailId,Phone,EmployeeName,EmployeeId,EmploymentLocation,EmployeeDesignation,Department,EmploymentStarted,EmploymentEnded".
        ") values('$rfirstname','$rlastname','$rorganization','$rdesignation','$remail','$rphone','$remployeeName',".
        "'$remployeeId','$rlocation','$rempDesignation','$rdepartment',CAST('". $rstartDate ."' AS DATE),CAST('". $rendDate ."' AS DATE))";
echo $sql; 
//CAST('". $date ."' AS DATE))"
//$selectQuery = "select * from tblContactus contactus where contactus.Email='$remail' and contactus.WorkPhone='$rphone'";
$selectQuery = "select * from tblEmpVerfication where Email='$remail' and Phone='$rphone'";
$selectresponce = mysql_query($selectQuery);
if(mysql_num_rows($selectresponce)>0){
            $resultMessage = '<font color=red size=2px>'.$remail.' is already exists</font>';
            ...
            $conn->closeConnection(); // closing the connection
              ...
              //  $mailmanager->sendContactForm($rfname, $rlname, $remail, $rorganization, $rdesignation, $rphone, $rcity, $rcountry, $rdescription);
                 $mailmanager->sendEmployeeVerificationDetails($rfirstname,$rlastname,$rorganization,$rdesignation,$remail,$rphone,$remployeeName,$remployeeId,$rlocation,$rempDesignation,$rdepartment);
      ...
      $exc->getMessage();
        // $resultMessage= $exc->getMessage();
          $resultMessage="<font color=red size=2px>".$exc.'</font>'.
                  ...
                  //$resultMessage= "We have an internal error,Please try later.";
                    $conn->closeConnection(); // closing the connection
